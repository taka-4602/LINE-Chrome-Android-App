    ...
    def _encrypt_e2ee_text(self, to: str, text: str,
        ...
        if not to.startswith("u"):
            raise NotImplementedError(
                "E2EE is only implemented for 1:1 chats. Group sealing needs "
                "the shared group key flow (registerE2EEGroupKey)."
            )
        ...
        receiver_key    = _as_bytes(_check(pub, 4) or b"")
        if receiver_key_id is None or len(receiver_key) != 32:
            raise RuntimeError(f"Bad peer key for {to}: {peer}")

        sender_key_id = int(self_key["keyId"])
        priv          = base64.b64decode(self_key["privKey"])

        shared  = _curve25519_shared(priv, receiver_key)
        salt    = os.urandom(16)
