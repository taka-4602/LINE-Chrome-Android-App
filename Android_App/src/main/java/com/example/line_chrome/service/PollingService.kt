package com.example.line_chrome.service
...
class PollingService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // Nothing starts this in demo mode, but START_STICKY means the system
        // can revive an instance left over from a real build — which would sit
        // there polling for a session that no longer exists.
        if (intent?.action == ACTION_STOP || LineRepository.DEMO) {
            stopSelf()
    ...
    companion object {
        ...
        private const val ACTION_STOP = "com.example.line_chrome.STOP_POLLING"
        ...
        fun stop(context: Context) {
            context.startService(
                Intent(context, PollingService::class.java).setAction(ACTION_STOP)
            )
        }
    }
}
