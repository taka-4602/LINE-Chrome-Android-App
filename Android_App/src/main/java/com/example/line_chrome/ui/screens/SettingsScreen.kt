...
fun SettingsScreen(
    ...
    Column(
        ...
        if (LineRepository.DEMO) DemoSection() else SignInSection()
        ...
        Text(
            text = if (LineRepository.DEMO) {
                "Unofficial client built by reverse-engineering LINE's desktop " +
                    "protocol. This build is running in demo mode on a fictional " +
                    "account, so nothing here reaches LINE and no real messages " +
                    "are involved."
            } else {
                "Unofficial client built by reverse-engineering LINE's desktop " +
        ...
        OutlinedButton(
            ...
            Text(if (LineRepository.DEMO) "Reset demo" else "Sign out")
        }
...
/**
 * Automatic re-login, which only means anything with a real session behind it.
 *
 * Extracted so demo mode can leave it out entirely rather than showing a
 * password switch for an account that does not exist.
 */
@Composable
...
/** What replaces the sign-in section when the app is running on demo data. */
@Composable
private fun DemoSection() {
    ...
    Text(
        "The account, the friends and every message in this build are made up, " +
            "and no request leaves the device. Replies and incoming messages are " +
            "scripted, so the app behaves as it would on a live session without " +
            "one being involved.",
        style = MaterialTheme.typography.bodySmall,
        color = MaterialTheme.colorScheme.onSurfaceVariant,
    )
...
private fun ConfirmLogoutDialog(
    ...
    if (visible) {
        AlertDialog(
            ...
            title = { Text(if (LineRepository.DEMO) "Reset the demo?" else "Sign out?") },
            text = {
                Text(
                    if (LineRepository.DEMO) {
                        "Puts the conversations back to how they started and drops " +
                            "anything sent during this run. Useful between demos."
                    } else {
            ...
            confirmButton = {
                TextButton(onClick = { onDismiss(); onLogout() }) {
                    Text(if (LineRepository.DEMO) "Reset" else "Sign out")
                }
            },
            dismissButton = {
