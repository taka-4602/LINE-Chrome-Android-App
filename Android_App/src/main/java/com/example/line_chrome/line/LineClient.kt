...
class LineClient(
    ...
    fun getProfile(): Profile = Profile.from(postTalk(EP_TALK, packCompact("getProfile", emptyList())))
    ...
    fun getAllContactIds(): List<String> {
        val res = postTalk(EP_TALK, packCompact("getAllContactIds", emptyList()))
        return tList(res, 0).filterIsInstance<String>()
    }

    private fun getAllChatMidsRaw(): Map<Int, Any?> = postTalk(EP_TALK, packCompact(
        "getAllChatMids",
    ...
    fun getChats(
        ...
        val targets = mids ?: getChatMids(includePeers)
        if (targets.isEmpty()) return emptyList()
        return targets.chunked(chunkSize).flatMap { batch ->
            val data = packCompact("getChats", fields {
                struct(1) { strList(1, batch); bool(2, withMembers) }
            })
            tList(postTalk(EP_TALK, data), 1).mapNotNull { c ->
                (c as? Map<*, *>)?.let { Chat.from(it) }
            }
        }
    }

    fun getContacts(mids: List<String>, chunkSize: Int = 200): List<Contact> =
        mids.chunked(chunkSize).flatMap { batch ->
            val data = packCompact("getContactsV2", fields { struct(1) { strList(1, batch) } })
            val res = postTalk(EP_TALK, data)
    ...
    fun getRecentMessages(chatMid: String, count: Int = 20): List<Message> {
        val data = packCompact("getRecentMessagesV2", fields { str(2, chatMid); i32(3, count) })
        val res = postTalk(EP_TALK, data)
    ...
    fun sendMessage(
        ...
        if (e2ee == true) {
            val sealed = postTalk(EP_TALK, build(encryptE2eeText(to, text, contentType)))
            return echoed(sealed, text)
        }

        val res = try {
            postTalk(EP_TALK, build(null))
        } catch (err: LineServiceError) {
            if (err.code != ERR_E2EE_RETRY_ENCRYPT || e2ee == false) throw err
            Log.i(TAG, "$to is letter-sealed; re-sending encrypted.")
            postTalk(EP_TALK, build(encryptE2eeText(to, text, contentType)))
        }
    ...
    private fun obsAccessToken(): String {
        ...
        val res = postTalk(EP_TALK, packCompact(
            "acquireEncryptedAccessToken", fields { i32(2, 2) }   // featureType = OBS
        ))
    ...
    fun unsendMessage(messageId: String) {
        postTalk(EP_TALK, packCompact("unsendMessage", fields { str(2, messageId) }))
    }

    fun sendReadReceipt(chatMid: String, lastMessageId: String) {
        postTalk(EP_TALK, packCompact("sendChatChecked", fields {
    ...
    fun getLastOpRevision(): Long {
        val res = postTalk(EP_TALK, packCompact("getLastOpRevision", emptyList()))
    ...
    fun getE2eePublicKeys(): List<Any?> =
        tList(postTalk(EP_TALK, packCompact("getE2EEPublicKeys", emptyList())), 0)
    ...
    fun negotiateE2eePublicKey(peerMid: String): Map<Int, Any?> =
        peerNegotiationCache.getOrPut(peerMid) {
            postTalk(EP_TALK, packCompact("negotiateE2EEPublicKey", fields { str(2, peerMid) }))
        }
    ...
    private fun groupSharedKey(groupMid: String, keyId: Int?): Pair<Int, ByteArray> {
        ...
        val res = postTalk(EP_TALK, packCompact("getLastE2EEGroupSharedKey", fields {
            i32(2, 2)                       // keyVersion
            str(3, groupMid)
        }))
    ...
    private fun encryptE2eeText(
        ...
        if (!to.startsWith("u")) {
            throw NotImplementedError(
                "E2EE is only implemented for 1:1 chats. Group sealing needs registerE2EEGroupKey."
            )
        }
