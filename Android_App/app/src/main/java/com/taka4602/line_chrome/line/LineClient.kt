...
class LineClient(
    ...
    fun sendMessage(
        ...
        // A group rotates its key whenever the membership changes, and we only
        // learn ours is stale by being refused with it — so drop the cached copy
        // and try once more with a freshly fetched one.
        fun postSealed() = try {
            post(build(encryptE2eeText(to, text, contentType)))
        } catch (err: LineServiceError) {
            if (err.code != ERR_OLD_GROUP_KEY) throw err
            Log.i(TAG, "$to refused our group key; refetching it and retrying.")
            post(build(encryptE2eeText(to, text, contentType, refreshGroupKey = true)))
        }

        if (e2ee == true) return echoed(postSealed(), text)
     ...
     *
     * [refresh] skips both caches.  A group key is rotated when the membership
     * changes, and with [keyId] unknown — which is the case when sending — the
     * caches would otherwise keep handing back the superseded one.
     */
    private fun groupSharedKey(
        groupMid: String, keyId: Int?, refresh: Boolean = false,
    ): Pair<Int, ByteArray> {
        if (!refresh) {
    ...
    /**
     * Encrypt a text message for a sealed chat, 1:1 or group.
     *
    ...
    private fun encryptE2eeText(
        to: String, text: String, contentType: Int, refreshGroupKey: Boolean = false,
        ...
        val priv: ByteArray
        val receiverKey: ByteArray
        val receiverKeyId: Int
        val specVersion: Int

        if (to.startsWith("c") || to.startsWith("r")) {
            // Groups do not do pairwise ECDH.  Everyone shares one key pair, so
            // we pair the group's *private* half with our own public half — the
            // same secret every member derives, and the same one e2eePayload
            // already computes when decrypting our own group messages.
            val group = groupSharedKey(to, null, refresh = refreshGroupKey)
            priv = group.second
            receiverKey = self.pubKey
            receiverKeyId = group.first
            ...
            specVersion = negotiated ?: 2

            receiverKeyId = tInt(peer, 2, 2) ?: error("Bad peer key for $to")
            receiverKey = asBytes(tGet(peer, 2, 4))
            if (receiverKey.size != 32) error("Bad peer key for $to")
            priv = self.privKey
        }

        val shared = Crypto.x25519Shared(priv, receiverKey)
