...
object Crypto {
    ...
    /**
     * The login plaintext: each field prefixed with its length in one byte.
     *
     * The prefix counts **bytes**, not characters.  Building this as a string
     * and prefixing `value.length` — as the reference client does — silently
     * mis-frames anything non-ASCII: a Japanese password declares 6 where it
     * sends 16, and the server rejects the blob as the wrong length.  For an
     * ASCII password the two are identical, which is why it takes a
     * non-English account to notice.
     *
     * A length of 128 or more would also stop fitting in one UTF-8 byte if this
     * went through a string, so it is assembled as raw bytes throughout.
     */
    fun credentialBlob(sessionKey: String, email: String, password: String): ByteArray {
        val out = java.io.ByteArrayOutputStream()
        for (field in listOf(sessionKey, email, password)) {
            val bytes = field.toByteArray(Charsets.UTF_8)
            require(bytes.size <= 255) {
                "Credential field is ${bytes.size} bytes; the length prefix holds 255"
            }
    ...
    fun rsaEncryptCredentials(
        ...
        val message = credentialBlob(sessionKey, email, password)
